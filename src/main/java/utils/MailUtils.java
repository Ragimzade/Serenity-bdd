        ...
        SearchTerm term = new SearchTerm() {
            ...
            public boolean match(Message message) {
                try {
                    ...
                    log.error(String.format("message with subject '%s' not found ", subject), ex);
                }
                return false;
            }
        };
        List<Message> messages = IBaseEntity.waitForList(TIMEOUT_IN_SECONDS, DELAY_IN_MILLIS,
                String.format("Message is with subject '%s'not found in %s folder", subject, folderName), () -> Arrays.asList(folder.search(term)));
